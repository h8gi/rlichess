...
`rlichess` provides an intuitive and tidy interface to the Lichess API (https://lichess.org/api) for chess players and data analysts working with R and tidyverse.
...
Lichess offers generous public API access. However, providing a Personal Access Token increases rate limits (up to 30–60 games/sec) and allows on-demand rating history retrieval.
...
Use `lic_get_games()` to fetch games for any Lichess user. You can filter by date, speed/perf, or opponent:
...
`lic_tidy_games()` enriches raw NDJSON records into a tidy format with readable dates (`POSIXct`), color (`"white"` / `"black"`), win/loss results, and rating differences:
...
For in-depth move-by-move analysis (evaluations, clock times, blunders), `lic_tidy_moves()` unpacks games into a long format where each row is a half-move (ply):
...
Query Lichess's vast opening database for candidate moves and win statistics:
